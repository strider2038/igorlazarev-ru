...
![null value for array in API](../images/nullable-array-in-api.png "К чему приводит использование null-значения для массива")

## Null и объекты

Одной из самых печально известных проблем в программировании считается проблема null pointer exception - [ошибка на миллиард долларов](https://www.infoq.com/presentations/Null-References-The-Billion-Dollar-Mistake-Tony-Hoare/).
...
* В качестве смыслового значения _"не заполнено"_ предпочтительнее вместо `null` использовать значение _"пустое"_, т.е.
  * `0` для числовых типов данных
  * пустую строку `""` для строковых типов
  * пустой массив `[]` для массивов
* Дважды подумайте стоит ли вводить возможность указывать `null` для атрибутов типа массив
